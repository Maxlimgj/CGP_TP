...
function onStart()
{
   ...
   initServer();
   initClient();
   $Editor::newMissionOverride = "tutorial.base/data/missions/project.mis";
}
...
function initClient()
{
   ...
   exec("./client/ui/instructions.gui");
...
function loadMyMission()
{
   ...
   createServer("SinglePlayer", expandFilename("./data/missions/project.mis"));
...
{
   Canvas.setContent(instructions);
   Canvas.setCursor("DefaultCursor");
}
